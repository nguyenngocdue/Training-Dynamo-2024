...
def getLocationElement(element, xyz='point'):
    point = element.Location.Point.ToPoint()
    if xyz == 'xyz': point = element.Location.Point
    return point

def sortPoints(points):
    sortedIndices = sorted(enumerate(points), key= lambda point: point[1].X)
    indices = [index for index, point in sortedIndices]
    sortedPoints = [point for index, point in sortedIndices]
